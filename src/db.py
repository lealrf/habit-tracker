...
class DatabaseManager:
    ...
    def get_all_habits(self) -> list[Habit]:
        ...
        # 1. Fetch all habits
        cursor.execute('SELECT id, name, description, periodicity, created_at FROM habits')
        ...
        for row in habit_rows:
            ...
            cursor.execute('SELECT timestamp FROM checkoffs WHERE habit_id = ?', (habit_id,))
            checkoff_rows = cursor.fetchall()
            
            # Use list comprehension to quickly parse all timestamps
            checkoff_dates =[datetime.fromisoformat(c_row[0]) for c_row in checkoff_rows]
