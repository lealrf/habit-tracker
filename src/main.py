"""Command-Line Interface (CLI) router and presentation layer."""
...
def add_habit(db: DatabaseManager):
    ...
    periodicity = questionary.select(
        "How often do you want to complete this habit?", choices=["daily", "weekly"]
    ).ask()
...
def checkoff_habit(db: DatabaseManager):
    ...
    selected_habit = questionary.select(
        "Which habit would you like to check off?", choices=choices
    ).ask()
...
def delete_habit(db: DatabaseManager):
    ...
    choices = [questionary.Choice(title=h.name, value=h) for h in habits]
    selected_habit = questionary.select(
        "Which habit do you want to delete?", choices=choices
    ).ask()

    # Confirm deletion
    confirm = questionary.confirm(
        f"Are you sure you want to delete '{selected_habit.name}'?"
    ).ask()
    if confirm:
...
def view_analytics(db: DatabaseManager):
    ...
    choices = [questionary.Choice(title=h.name, value=h) for h in habits]
    selected_habit = questionary.select(
        "Select a habit to analyze:", choices=choices
    ).ask()
